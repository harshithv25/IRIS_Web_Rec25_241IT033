      ...
      {equipment?.length !== 0 ? (
        <>
          {Array.from({ length: totalPages }).map((_, pageIndex) => (
            <section key={pageIndex} id={`section-${pageIndex}`} className="w-full min-h-screen flex flex-col justify-center items-center snap-start">
              <div className="p-6 md:p-8 rounded-xl backdrop-blur-xs flex flex-col lg:flex-row justify-center items-center gap-10 shadow-lg w-full max-w-full text-white">
                {equipment?.slice(pageIndex * itemsPerPage, (pageIndex + 1) * itemsPerPage)?.map((item) => (
                  <div
                    key={item._id}
                  ...
                  >
                    ...
                    <p className="flex items-center gap-2 text-lg">
                      ...
                      <span className="text-gray-400 flex items-center justify-center gap-2 font-bold">{capitalize(item?.category)}</span>
                    </p>
                    <p className="flex items-center gap-2 text-lg">
                      <span className="text-gray-500 flex items-center justify-center gap-2 font-semibold">
                        <Box /> Quantity:
                      </span>
                      <span className="text-gray-400 flex items-center justify-center gap-2 font-bold">{capitalize(`${item?.quantity}`)}</span>
                    </p>
                    <p className="flex items-center gap-2 text-lg">
                      <span className="text-gray-500 flex items-center justify-center gap-2 font-semibold">
                        <Heart /> Condition:
                      </span>
                      <span className="text-gray-400 flex items-center justify-center gap-2 font-bold">{capitalize(`${item?.condition}`)}</span>
                    </p>
                    <div className="text-lg w-full">
                      ...
                      <ol className="list-decimal ml-5 text-sm h-30 overflow-y-scroll">
                        ...
                        {Object.entries(item?.operating_hours || {}).map(([day, slots]) => (
                          <li key={day} className="mt-1 font-semibold text-gray-400">
                            {capitalize(day)}
                    ...
                    >
                      <span className="bg-clip-text text-transparent bg-gradient-to-r from-purple-500 to-blue-500">Edit</span>
                    </button>
                  </div>
                ))}
              </div>
        ...
        <div className="w-full min-h-screen flex flex-col gap-10 justify-center items-center snap-start">
          <h1 className="text-4xl font-bold">Oops! there are no equipments!</h1>
          <button
          ...
          >
            <span className="bg-clip-text text-transparent bg-gradient-to-r from-purple-500 to-blue-500">Create equipment!</span>
          </button>
        </div>
      )}
    </div>
